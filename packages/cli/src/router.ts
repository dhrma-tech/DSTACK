...
export async function route(command: ParsedCommand): Promise<{ stdout: string; stderr: string; exitCode: number }> {
  ...
  const config = await ConfigManager.load({ projectRoot, cliModel: command.invocation?.flags.model ?? null, allowSecrets: command.invocation?.flags.allowSecrets ?? false });
  ...
  return { stdout: resultText(await executor.run(command.invocation)), stderr: "", exitCode: 0 };
}
