...
import { ModelRouter, StreamHandler } from "./model.js";
...
export class SkillExecutor {
  ...
  async run(invocation: SkillInvocation): Promise<SkillRunResult> {
    ...
    try {
      ...
      enforceWorkflowGates(manifest, prerequisiteArtifacts, invocation.flags.force);
      ...
      const context: SkillExecutionContext = { manifest, invocation, config: this.options.config, artifactStore: this.artifacts, memoryStore: this.memory, checkpointStore: this.checkpoints, toolExecutor, prerequisiteArtifacts };
      ...
      const output = await handler.postProcess(rawOutput, context);
...
export const contextSaveHandler: SkillHandler = {
  ...
  async run(context) {
    ...
    const output = checkpoint as unknown as JsonObject;
...
export const contextRestoreHandler: SkillHandler = {
  ...
  async run(context) {
    ...
    const output: JsonObject = { checkpointName: checkpoint.name, restoredAt: new Date().toISOString(), artifactsRestored: Object.keys(checkpoint.artifactPointers), gitHeadAtSave: checkpoint.gitHead, note: "Artifact pointers restored. Git state was not modified." };
    const artifact = context.invocation.flags.dryRun ? null : await context.artifactStore.write(context.manifest.name, output);
    return { skillName: context.manifest.name, status: "complete", verdict: "PASS", artifactPath: artifact?.filePath ?? null, output, nextSkill: context.manifest.nextSkill, warnings: [] };
  }
};
...
function enforceWorkflowGates(manifest: SkillManifest, artifacts: Record<string, JsonObject>, force: boolean): void {
  if (force) return;
